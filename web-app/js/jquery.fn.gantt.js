    ...
    $.fn.gantt = function (options) {
        ...
        */
        ...
        var core = {
            ...
            createProgressBar1: function (days, cls, desc, label, dataObj) {
              //  alert("sanAlert"+""+days+""+cls+""+desc+""+label+""+dataObj)
            	var cellWidth = tools.getCellSize();
               var desc="";
               var label="";
            	var barMarg = tools.getProgressBarMargin() || 0;
                var bar = $('<div class="bar1"><div class="fn-label">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
                            width: ((cellWidth * days) - barMarg) + 2
                        })
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                 ...
                                 //  var sanFrom=tools.genId(tools.dateDeserialize(day.actualStartDate).getTime(), element.scaleStep);
                                  //  alert(sanFrom);
                                ...
                                default:
                                    ...
                                    var sanFrom=tools.genId(tools.dateDeserialize(day.actualStartDate).getTime());//santhosh
                                   var sanTo=tools.genId(tools.dateDeserialize(day.actualEndtDate).getTime());//santhosh
                                    var from = $(element).find("#dh-" + dFrom);
                                    var cFrom = from.attr("offset");
                                    var from1=$(element).find("#dh-" + sanFrom);//santhosh
                                    var cFrom1=from1.attr("offset");//santhosh
                                    var dl = Math.floor(((dTo / 1000) - (dFrom / 1000)) / 86400) + 1;
                                   var d2=Math.floor(((sanTo / 1000) - (sanFrom / 1000)) / 86400) + 1;//santhosh
                                    //alert(sanFrom+""+sanTo+""+from1+""+cFrom1+""+d2);
                                    _bar = core.createProgressBar(
                                                dl,
                                                day.customClass ? day.customClass : "",
                                                day.desc ? day.desc : "",
                                  ...
                                  _bar1=core.createProgressBar1(
                                    ...
                                    _bar.css({ 'top': top, 'left': Math.floor(cFrom) });
                                   _bar1.css({ 'top': top+5, 'left': Math.floor(cFrom1) });
                                    datapanel.append(_bar);
                                   datapanel.append(_bar1);
                                    break;
                            }
