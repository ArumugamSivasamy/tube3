...
// Added Check Date 
	jQuery.validator.addMethod("greaterThan", 
			function(value, element, params) {
				...
				if (!/Invalid|NaN/.test(new Date($(params).val()))) {
			        return new Date(value) <= new Date($(params).val());
			    }
                
			    return isNaN(value) || (parseFloat(value) <= parseFloat($(params).val())); 
			},'Must be less than planned end date.');
			
	
	jQuery.validator.addMethod("calenderGreaterThan", 
			function(value, element, params) {
...
jQuery.validator.addMethod("checkRange", function(value, element, params) {
	var dateDiff;
	var maximumHours;
	//dateDiff=(new Date($(params[1]).val()).getDate())-(new Date($(params[0]).val()).getDate());
	//dateDiff=dateDiff*8;
	//alert()
	if (!/Invalid|NaN/.test(value)) {
		// alert($(params[0]).val());
		dateDiff = (new Date($(params[1]).val()).getDate())
				- (new Date($(params[0]).val()).getDate());
		maximumHours = (dateDiff + 1) * 8;
		//alert(maximumHours);
		return value <= maximumHours;
	}

	return isNaN(value) && (value <= maximumHours);
}, 'Check Estimated Hour Exceed than working Hour ');

jQuery.validator.addMethod("workingDays", function(value, element, params) {
	var dateDiff;
	var maximumHours;
	if (!/Invalid|NaN/.test(value)) {
		var days = calcBusinessDays(new Date($(params[0]).val()), new Date($(
				params[1]).val()));
		maximumHours = days * 8;
		alert(maximumHours);
		return value <= maximumHours;
...
$.validator.addMethod("parentLessThan", function(value, element, param) {
	var $min = $(param);
...
}, "Estimated hour must be less than Estimated Avaliable Hours");
...
$.validator.addMethod("lessThan", function(value, element, param) {
	...
	return parseInt(value) <= parseInt($min.val()) && parseInt(value) <= parseInt($max.val()) && parseInt(value) != 0;
}, "Estimated hour must be less than SubTask/Estimated Avaliable Hours");

$.validator.addMethod("checkDateValue", function(value, element, param) {
	var $min = $(param);
	var start = $min.val();
	var end = value;
	var s = start.split(':');
	...
	return hour > 0;
}, "Worked hours should not be zero");
...
jQuery.validator.addMethod("checkAvailable", function(value, element) {
	...
	return value > 0;
}, "Estimated hour is zero so cant able to create subtask");
